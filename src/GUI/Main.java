...
import TestAPI.AddNhanVienHandler;
import TestAPI.DeleteNhanVienHandler;
import TestAPI.GetNhanVienHandler;
import TestAPI.UpdateNhanVienHandler;
...
private void startHttpServer() {
    ...
    executor.submit(() -> {
        try {
            // cổng 8081 api test http://localhost:8081/test
            HttpServer server = HttpServer.create(new InetSocketAddress(8081), 0);
            server.createContext("/post", new AddNhanVienHandler()); 
            server.createContext("/get", new GetNhanVienHandler()); 
            server.createContext("/put", new UpdateNhanVienHandler()); 
            server.createContext("/delete", new DeleteNhanVienHandler()); 
            server.setExecutor(null);
            System.out.println("Server đang chạy tại http://localhost:8081");
            server.start();
        } catch (IOException e) {
            System.err.println("Không thể khởi động HTTP Server: " + e.getMessage());
        }
    });
}
